...
class UserProfile extends Component {
  ...
  checkout = (id) => {
    var warning = "";
    if (this.state.checkout_reason == "") {
      warning = "Please Select Reason";
    } else if (
      !(
        this.state.focal_contact.match("^[0-9]*$") && this.state.focal_contact != ""
      )
    ) {
      warning = "Please Enter Valid Focal Contact";
    }  else {
      warning = "";
    }
    if (warning) {
    ...
    new Apimanager()
      .postroute("warehouse_manager/item_checkout", {
        item: this.state.checkoutitem._id,
        checkout_reason: this.state.checkout_reason,
        focal_contact: this.state.focal_contact,
      })
      ...
      .then(() => {
        new Apimanager()
          .postrouteFormdata(
            `warehouse_manager/item_checkout/upload/${this.state.checkoutitem.item_id}/${this.state.checkout_id}`,
            Formdata
          )
          .then((res) => {
            if (res.data) {
              this.setState(prevState=>{
                return{
                  ...
                  items:prevState.items.map(item=>{
                    console.log(JSON.stringify(this.state.checkoutitem.item_id)+item.item_det.item_id+prevState.checkoutitem.item_id+'item')
                    if(item.item_det.item_id==prevState.checkoutitem.item_id){
                          return{
                            ...item,
                            checked:true
                          }
  ...
  editrecord = (item) => {
    this.setState({ modalcheckitem: true, checkoutitem: item.item_det });
  };
  componentDidMount = () => {
    if (this.props.location.data && !this.state.ispageloaded) {
      ...
      this.setState({
        items: data.items,
        isorder: data.isorder ? true : false,
  ...
  submitform = () => {
    new Apimanager()
      .PutrouteByid(`customers/place_order/${this.state.order_id}`, {
        status: true,
      })
      .then(
        (res) => {
  ...
  render() {
    return (
      <div className="content">
        <Grid fluid>
                ...
                content={
                  <form>
                    <Row>
                            ...
                            {this.state.items.map((item, key) => {
                              return (
                                ...
                                >
                                  ...
                                  {!this.state.isorder&&
                                     ( <td style={{ textAlign: "center" }}>
                                  {(item.item_det.avalibility) && ( <Button
                                          onClick={() =>
                                            this.editrecord(item, key)
                                          }
                                        >
                                          Checkout
                                        </Button>)}
                      ...
                      {!this.state.isorder && (!this.state.done) && (
                        <Col md={12}>
                          <Button
          ...
          <Modal show={this.state.modalShow}>
              ...
              <Col md={12}>
                <Card
                  ...
                  content={
                    <Row>
                      <Col md={4}>
                        {/* //<QrCode value={this.state.item.item_id} /> */}
                        {/* <div>{this.state.item.item_id}</div> */}
                      </Col>
                      <Col md={6}>
                        <Image
            ...
            <div style={{ height: 60, width: "38%", marginLeft: "31%" }}>
              <Button
                variant="primary"
                onClick={() => this.setState({ modalShow: false, item: {} })}
              >
                Ok
              </Button>
            </div>
        ...
        <Modal show={this.state.modalcheckitem} style={{ marginTop: -30 }}>
          <div className="content">
            <Grid fluid>
              <Row>
                <Col md={12}>
                      ...
                      <form>
                        <Row>
                          ...
                          <Col xs={6}>
                            ...
                            <Image
                              src={`${config.url}public/images/${this.state.checkoutitem.picture_pathway}`}
                              roundedCircle
                              style={{
                                width: 200,
                                height: 200,
                          ...
                          <FormInputs
                            ...
                            properties={[
                              ...
                              },
                              {
                                ...
                                onChange: (evt) => {
                                  this.setState({
                                    focal_contact: evt.target.value,
                                  });
                                },
                              },
                            ]}
