...
class UserProfile extends Component {
  ...
  submitform = () => {
    ...
    new Apimanager()
      .PutrouteByid(`Warehouse_manager/item_induction/${this.state.item_id}`, {avalibility:true,recievedstatus:true})
      .then((res) => {
        if (res.status == 200) {
            new Apimanager()
            .postrouteFormdata(
              `Warehouse_manager/item_induction/update/upload/${this.state.item_id}`,
              Formdata
            )
            .then((res) => {
              if (res.status == 200) {
                this.setState({
                  ...
                  alert: (
                    <SweetAlert
                      success
                      style={{ display: "block", marginTop: "100px" }}
                      title="Successfully Inducted Item!"
                      onConfirm={() => this.setState({ alert: null })}
                      onCancel={() => this.setState({ alert: null })}
                      confirmBtnBsStyle="info"
                    ></SweetAlert>
  ...
  render() {
    // console.log(JSON.stringify(this.state.item_comments)+"state")
    return (
      <div className="content">
                ...
                content={
                  <form>
                    <Row >
                      ...
                      <Col xs={6} md={4}>
                        <Image
                          src={this.state.img?this.state.img:`${config.url}public/images/${this.state.picture_pathway}`}
                          roundedCircle
                    ...
                    <FormInputs
                      ...
                      properties={[
                        ...
                        
                        {
                          ...
                          value: this.state.item_comments,
                          bsClass: "form-control",
                          disabled: true,
                      ...
                      <Col xs={6}>
                        ...
                        <div>
                          ...
                          value={this.state.item_cat}
                            bsStyle="default"
                            bsSize="large"
                            title={this.state.item_cat.cat_name || "Select A Category"}
                          >
                          </DropdownButton>
                        </div>
                      </Col>
                    ...
                    {!this.state.returnedorder&&      <Button
        ...
        <Modal style={{width:'20%',height:'60%',borderRadius:40,marginLeft:'40%'}} size="sm" show={this.state.save}>
          <Modal.Header onHide={()=>this.setState({save:false})} closeButton>
          </Modal.Header>
          <Row>
            <Col md={12}>
              <Card
...
<div style={{alignSelf:'center',marginLeft:'15%'}}>
    ...
    <Button variant="secondary" onClick={()=>this.setState({save:false})}>Close</Button>
           </span>
        </div>
      </Modal>
        {this.state.alert}
