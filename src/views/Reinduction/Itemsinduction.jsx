...
class Closeit extends React.Component {
  ...
  editrecord = (item, key) => {
    this.props.history.push({
      pathname: `reinduction/items`,
      data: this.state.details[key],
    });
  };

  componentDidMount() {
    new Apimanager()
      .Getroute("warehouse_manager/item_induction/maintainance")
      .then((res) => {
        // console.log(res+'res')
        //   res.shift()
  ...
  render() {
    var dataTable = {
      ...
      dataRows: this.state.details.map((item) => [
        ...
        item.item_brand,
        item.item_colour,
        item.checkout_det.checkout_id,
        item.checkout_det.checkout_reason,
        item.checkout_det.focal_contact,
      ]),
    };
    return (
