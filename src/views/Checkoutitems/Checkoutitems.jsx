...
class Closeit extends React.Component {
  ...
  editrecord = (item,key) => {
    this.setState({ modalcheckitem: true, checkoutitem: this.state.details[key] });
  };
  componentDidMount() {
    new Apimanager()
      .Getroute("warehouse_manager/item_induction")
      .then((res) => {
 res =  res.filter(item=>item.item_name?true:false)
 //   res.shift()
        this.setState((prevState) => {
          return {
            ...prevState,
  ...
  render() {
    var dataTable = {
      headerRow: ["Item Id", "Item Name", "Item Brand", "Item Colour"],
