...
class Closeit extends React.Component {
  ...
  componentDidMount() {
    new Apimanager().Getroute('warehouse_manager/item_induction').then((res)=>  {
  ...
  render() {
    ...
    var dataTable = {
      ...
      dataRows:  this.state.details.map(item=>[item.item_id,item.item_name,item.item_brand,item.item_colour])
    };
    return (
